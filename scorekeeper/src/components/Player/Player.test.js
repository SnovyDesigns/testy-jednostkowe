...
it('should call onPlayerRemove with player ID', () => {
  const playerIdPassed = 0;
  const mockedOnPlayerRemove = jest.fn();
  const playerComponent = shallow(
    <Player onPlayerRemove={mockedOnPlayerRemove} id={playerIdPassed} />
  );

  const deleteButton = playerComponent.find('.Player__delete');
  deleteButton.simulate('click');

  expect(mockedOnPlayerRemove).toBeCalledWith(playerIdPassed);
});
